...
int fill_stream_info(AVStream *avs, AVCodec **avc, AVCodecContext **avcc)
{
	*avc = avcodec_find_decoder(avs->codecpar->codec_id);
	if (!*avc)
	{
		err_log("failed to find the codec");
		return -1;
	}
	...
	*avcc = avcodec_alloc_context3(*avc);
	if (!*avcc)
	{
		err_log("failed to alloc memory for codec context");
		return -1;
	}

	if (avcodec_parameters_to_context(*avcc, avs->codecpar) < 0)
	{
		err_log("failed to fill codec context");
		return -1;
	}

	if (avcodec_open2(*avcc, *avc, NULL) < 0)
	{
		err_log("failed to open codec");
		return -1;
	}
	return 0;
}
...
int open_media(const char *in_filename, AVFormatContext **avfc)
{
	*avfc = avformat_alloc_context();
	if (!*avfc)
	{
		err_log("failed to alloc memory for format");
		return -1;
	}

	if (avformat_open_input(avfc, in_filename, NULL, NULL) != 0)
	{
		err_log("failed to open input file %s", in_filename);
		return -1;
	}

	if (avformat_find_stream_info(*avfc, NULL) < 0)
	{
		err_log("failed to get stream info");
		return -1;
	}
	return 0;
}
...
int prepare_decoder(StreamingContext *sc)
{
	for (int i = 0; i < sc->avfc->nb_streams; i++)
	{
		...
		{
			dbg_log("skipping streams other than audio and video");
		}
	}
...
int prepare_video_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, AVRational input_framerate, StreamingParams sp)
{
	...
	if (!sc->video_avc)
	{
		err_log("could not find the proper codec");
		return -1;
	}
	...
	if (!sc->video_avcc)
	{
		err_log("could not allocated memory for codec context");
		return -1;
	}
	...
	{
		err_log("could not open the codec");
		return -1;
	}
...
int remux(AVPacket **pkt, AVFormatContext **avfc, AVRational decoder_tb, AVRational encoder_tb)
{
	...
	{
		err_log("error while copying stream packet");
		return -1;
	}
	return 0;
}
...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame)
{
	...
	if (!output_packet)
	{
		err_log("could not allocate memory for output packet");
		return -1;
	}
	...
	while (response >= 0)
	{
		...
		{
			err_log("Error while receiving packet from encoder: %s", av_err2str(response));
			return -1;
		}
		...
		if (response != 0)
		{
			err_log("Error %d while receiving packet from decoder: %s", response, av_err2str(response));
			return -1;
		}
	}
...
int transcode_audio(StreamingContext *decoder, StreamingContext *encoder, AVPacket *input_packet, AVFrame *input_frame)
{
	...
	if (response < 0)
	{
		err_log("Error while sending packet to decoder: %s", av_err2str(response));
		return response;
	}
	...
	while (response >= 0)
	{
		...
		{
			err_log("Error while receiving frame from decoder: %s", av_err2str(response));
			return response;
		}
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
	{
		err_log("Inciffent number of arguments. Help: ./a.out inputfile outputfile");
		return -1;
	}
	/*
   * H264 -> H264 (fixed gop)
   ...
   */
	...
	if (!encoder->avfc)
	{
		err_log("could not allocate memory for output format");
		return -1;
	}
	...
	if (!(encoder->avfc->oformat->flags & AVFMT_NOFILE))
	{
		...
		{
			err_log("could not open the output file");
			return -1;
		}
	}
	...
	{
		err_log("an error occurred when opening output file");
		return -1;
	}
	...
	if (!input_frame)
	{
		err_log("failed to allocated memory for AVFrame");
		return -1;
	}
	...
	if (!input_packet)
	{
		err_log("failed to allocated memory for AVPacket");
		return -1;
	}
	...
	while (av_read_frame(decoder->avfc, input_packet) >= 0)
	{
		...
		{
			dbg_log("ignoring all non video or audio packets");
		}
	}
